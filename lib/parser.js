...
var ORIGINAL_SUBJECT_LAX_REGEXES = [
...
var parseSubject = function(subject) {
  var _match = loopRegexes(SUBJECT_REGEXES, subject)
 ...
 * @return {object}  The result
 */
var parseBody = function(body, forwarded = false) {
  ...
  var _match = loopRegexes(SEPARATOR_REGEXES, _body, "split")
  ...
  if (forwarded === true) {
    ...
    _match = loopRegexes(ORIGINAL_FROM_REGEXES, _body, "split");
 ...
 */
var parseOriginalEmail = function(text, body) {
  // Remove ">" at the beginning of each line, while keeping line breaks
  var _text = text.replace(QUOTE_LINE_BREAK_REGEX, "");
  ...
  return {
    body    : __parseOriginalBody(_text),

    from    : __parseOriginalFrom(_text, body),
    to      : __parseOriginalTo(_text),
    cc      : __parseOriginalCc(_text),

    subject : __parseOriginalSubject(_text),
    date    : __parseOriginalDate(_text, body),
  }
}
...
var __parseOriginalBody = function(text) {
  ...
  //   Yahoo Mail)
  _match = loopRegexes(
    ORIGINAL_SUBJECT_REGEXES.concat(ORIGINAL_SUBJECT_LAX_REGEXES),
...
var __parseOriginalFrom = function(text, body) {
  ...
  _match = loopRegexes(SEPARATOR_REGEXES, body)
  ...
  //   (Yahoo Mail)
  _match = loopRegexes(ORIGINAL_FROM_LAX_REGEXES, text);
...
var __parseOriginalTo = function(text) {
  ...
  //   (Yahoo Mail)
  var _cleanText = loopRegexes(ORIGINAL_SUBJECT_LAX_REGEXES, text, "replace");
  _cleanText = loopRegexes(ORIGINAL_DATE_LAX_REGEXES, _cleanText, "replace");
  _cleanText = loopRegexes(ORIGINAL_CC_LAX_REGEXES, _cleanText, "replace");

  return __parseOriginalRecipients(ORIGINAL_TO_LAX_REGEXES, _cleanText);
}
 ...
 */
var __parseOriginalCc = function(text) {
  ...
  var _recipients = __parseOriginalRecipients(ORIGINAL_CC_REGEXES, text);
  ...
  return __parseOriginalRecipients(ORIGINAL_CC_LAX_REGEXES, _cleanText);
}
 ...
 */
var __parseOriginalRecipients = function(regexes, text) {
...
var __parseOriginalSubject = function(text) {
  ...
  //   regexes (Yahoo Mail)
  _match = loopRegexes(ORIGINAL_SUBJECT_LAX_REGEXES, text);
...
var __parseOriginalDate = function(text, body) {
  ...
  _match = loopRegexes(ORIGINAL_DATE_LAX_REGEXES, _cleanText);
...
exports.parseOriginalEmail = parseOriginalEmail;

exports.default = {
  parseSubject       : parseSubject,
  parseBody          : parseBody,
  parseOriginalEmail : parseOriginalEmail
};
