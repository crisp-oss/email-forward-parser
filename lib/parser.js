  ...
  parseBody(body, forwarded = false) {
    ...
    if (forwarded === true) {
      ...
      if (_match && _match.length > 3) {
        // The `split` operation creates a match with 4 parts:
        //  * 0: the message before the original email
        //  * 1: the line that contains the From part in the original email
        //  * 2: the From part itself (because we use the 'original_from' regex)
        //  * 3: the rest of the original email

        // Attach the line that contains the From part back to the rest of the \
        //   original email (exclude the From part itself)
        var _email = _match[1] + _match[3];

        // Notice: in case of nested emails, there may be several matches \
        //   against 'original_from'. In that case, the `split` operation \
        //   creates a match with (n x 4) parts.
        if (_match.length > 4) {
          for (var _i = 4; _i < _match.length; _i++) {
            // Exclude the From part itself
            if (_i % 3 !== 2) {
              _email += _match[_i];
            }
          }
