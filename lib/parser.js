...
var MULTIPLE_MAILBOXES_SEPARATORS = [
...
class Parser {
  ...
  __parseOriginalTo(text) {
    ...
    var _recipients = this.__parseMailbox(this.__regexes.original_to, text);

    // Recipient(s) found?
    if (Array.isArray(_recipients) ||
      !(_recipients.address === null && _recipients.name === null)
    ) {
      return _recipients;
    }
    ...
    return this.__parseMailbox(this.__regexes.original_to_lax, _cleanText);
  }
  ...
  __parseOriginalCc(text) {
    ...
    //   (Apple Mail, Gmail, Outlook Live, New Outlook 2019, Thunderbird)
    var _recipients = this.__parseMailbox(this.__regexes.original_cc, text);
    ...
    return this.__parseMailbox(this.__regexes.original_cc_lax, _cleanText);
  }
  ...
  __parseMailbox(regexes, text) {
    ...
    if (_match && _match.length > 0) {
      ...
      if (_mailboxesLine) {
        var _mailboxes = loopRegexes(
          MULTIPLE_MAILBOXES_SEPARATORS,
          _mailboxesLine,

          "split"
        );

        // Extract name and address (single / multiple mailboxes)
        for (var _i = 0; _i < _mailboxes.length; _i++) {
          var _nameAddressMatch = loopRegexes(
            this.__regexes.name_address,
            trimString(_mailboxes[_i])
          );

          // Name and address available?
          if (_nameAddressMatch && _nameAddressMatch.length > 0) {
            var _address = trimString(_nameAddressMatch[2]);
            var _name = trimString(_nameAddressMatch[1]);
            ...
            _mailboxes[_i] = {
              address : trimString(_mailboxes[_i]),
              name    : null
            };
          }
        }
